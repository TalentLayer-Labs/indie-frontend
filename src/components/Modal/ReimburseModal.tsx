...
function ReimburseModal({ service, payments, isSeller }: IPaymentModalProps) {
  const [show, setShow] = useState(false);
  const rateToken = service.validatedProposal[0].rateToken;
  const rateAmount = service.validatedProposal[0].rateAmount;
  const network = useNetwork();

  const totalReimbursements = payments.reduce((acc, payment) => {
    return acc.add(ethers.BigNumber.from(payment.amount));
  }, ethers.BigNumber.from('0'));

  const totalInEscrow = ethers.BigNumber.from(rateAmount).sub(totalReimbursements);
  ...
  return (
    <>
      <button
        ...
        className={`block ${
          (service.status === ServiceStatusEnum.Confirmed && !isSeller) ||
      ...
      <div
        ...
        <div className='relative p-4 w-full max-w-2xl h-auto'>
          <div className='relative bg-white rounded-lg shadow '>
            ...
            {isSeller && totalInEscrow.gt(0) && show && (
              <ReleaseForm
